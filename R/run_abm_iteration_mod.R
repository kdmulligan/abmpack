...
#' @param t_hcw_room transmission between hcw to room prior value
...
run_abm_iteration_mod <- function(n_days = 72,
                              t_symp_room = 0.0005, t_hcw_room = 0.0005,
                              t_room_patge65 = 0.0005,
  ...
  for (d in 1:total_days_sim) {
    ...
    if(d %in% c(1, 38, 39, 59, 60)) {browser()}
    ...
    for (tb in 1:4) {
      ...
      hcw_asymp_prob = tb_hcw_room_inter |>
        mutate(
          prob_hcw_asymp_rm = if_else(rid_uniq %in% idx_room_contam, (1 - exp(-total_sec * tau_bn_hcw_room)), 0)
        ) |>
